...
class GatherOperation(nn.Module):
    def execute(self, features, idx):
        B, C, N = features.shape
        B, npoint = idx.shape
        return features.reindex([B,C,npoint], ['i0','i1','@e0(i0,i2)'], extras=[idx])

class GroupingOperation(nn.Module):
    def execute(self, features, idx):
        B, C, N = features.shape
        B, npoint, nsample = idx.shape
        return features.reindex([B,C,npoint,nsample], ['i0','i1','@e0(i0,i2,i3)'], extras=[idx])
...
class BallQueryGrouper(nn.Module):
    ...
    def execute(self, new_xyz, pointset, feature):
        ...
        block_size = optimal_block(n_input)
...
def optimal_block(n):
    return min(2 ** int(math.log(n) / math.log(2)+1e-6), 512)
...
class FurthestPointSampler(nn.Module):
    cuda_src = '''
        ...
        furthest_point_sampling_kernel<<<in0_shape0, block_size>>>(
            in0_shape0,
            in0_shape1,
            out_shape1,
            in0_p,
    ...
    def execute(self, x):
        ...
        block_size = optimal_block(n_points)

        cuda_src = self.cuda_src.replace('block_size', str(block_size))

        idxs_shape = [batch_size, self.n_samples]
        tmp = jt.array([1e10]).broadcast([x.shape[0], x.shape[1]])
        idxs = jt.code(idxs_shape, 'int32', [x, tmp], cuda_src=cuda_src)
